    ...
    *******************************************************************************/
    ...
    public function userEdit()
    {
        ...
        if(strval($_POST['form_timestamp']) == strval($session->orig_timestamp))
        {
            ...
            $priv_user = $_POST['cbo_priv'];
            $password_nuevo_a = $_POST['password_nuevo_a'];
            $password_nuevo_b = $_POST['password_nuevo_b'];
    ...
    public function privilegiosAdd()
    {
        ...
        if(strval($_POST['form_timestamp']) == strval($session->orig_timestamp))
        {
            ...
            if($error[0] == 00000){
                ...
                $n_modules = $model->getLastModuleCodeNumber();
                ...
                for($i=1; $i<=$n_modules; $i++){
                    for($j=1; $j<=3; $j++){
                        ...
                        $model->updatePrivilegePermission($cod_privilegio, $i, $val_ver, $val_agregar, $val_editar);
                    }
                }
                ...
                $module_opt = $model->getModuleCodeByName("exportar");
                $model->updatePrivilegePermission($cod_privilegio, $module_opt, $option, $option, $option);
                ...
                if($error[0] == 00000)
                    $this->privilegiosPanel(11,"Cambios guardados en forma exitosa.");
                else
...
//                    $this->privilegiosPanel(10, "Ha ocurrido un error: <i>".$error[2]."</i>");
    ...
    public function privilegiosChange()
    {
        ...
        if(strval($orig_timestamp) == strval($session->orig_timestamp))
        {
            ...
            if(isset($_POST['cod_privilegio']))
            {
                ...
                #$n_modules = $model->getNumberOfModulesByStatus(1);
                $n_modules = $model->getLastModuleCodeNumber();
                ...
                for($i=1; $i<=$n_modules; $i++){
                    for($j=1; $j<=3; $j++){
                        ...
                        $model->updatePrivilegePermission($privi, $i, $val_ver, $val_agregar, $val_editar);
                    }
                }
                ...
                $module_opt = $model->getModuleCodeByName("exportar");
                $model->updatePrivilegePermission($privi, $module_opt, $option, $option, $option);
                
                // modulo menu tipo usuario            
                $module_menu = $model->getModuleCodeByName("mimenu");
                $module_menuuser = $model->getModuleCodeByName("mimenuuser");
                
                if($module_menu == $_POST['radio_menu']){
                    $model->updatePrivilegePermission($privi, $module_menu, 1, 1, 1);
                    $model->updatePrivilegePermission($privi, $module_menuuser, 0, 0, 0);
                }
                else{
                    $model->updatePrivilegePermission($privi, $module_menu, 0, 0, 0);
                    $model->updatePrivilegePermission($privi, $module_menuuser, 1, 1, 1);
                }
    ...
    */
    public function verifyNamePrivilege()
    {
        ...
        {
            $input = $_REQUEST['name_privilegio'];
