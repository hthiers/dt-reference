	...
	*******************************************************************************/
        ...
        public function ajaxClientesDt()
        {
            ...
            */
            ...
            if( isset($_GET['filTipo']) && $_GET['filTipo'] != "")
            {
                ...
                $sWhere .= " A.TIPO LIKE '%".$_GET['filTipo']."%' ";
            }
	...
	public function clientesAddForm($error_flag = 0)
	{
		...
		$model = new ClientesModel();
                ...
                $newCode = $model->getNewClienteCode();
                
                if($code = $newCode->fetch(PDO::FETCH_ASSOC))
		{
                    //Crear un nuevo codigo: anterior+1
                    $NUEVO_CODIGO = preg_replace("/[A-Za-z]/", "", $code['COD_CLIENTE']);
                    $LETRAS = preg_replace("/[0-9]/", "", $code['COD_CLIENTE']);  
