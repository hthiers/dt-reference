                ...
                <tbody>
                    ...
                    while($row = $lista_modulos->fetch(PDO::FETCH_ASSOC))
                    {
                        ...
                        else
                        {
                            ...
                            if($row['PARENT_MODULO'] != null && $row['PARENT_MODULO'] > 0){
                                echo "<td><span class='ui-icon ui-icon-carat-1-sw' style='float:left;'></span>".$row['LABEL_MODULO']."</td>";
                        
                                echo "<td class='options'> \n";
                                    echo "<input type='checkbox' class='chk_ver' title='ver registros' id='optver_".$row['NAME_MODULO']."' name='access[".$row['COD_MODULO']."][1]' />"; 
                                echo "</td>";
                            }
                            else{
                                echo "<td>".$row['LABEL_MODULO']."</td>";
                                ...
                                echo "<td class='options'> \n";
                                    echo "<input type='checkbox' class='chk_ver' title='solo vista' id='optver_".$row['NAME_MODULO']."' name='access[".$row['COD_MODULO']."][1]' /> ver"; 
                                    echo "<input type='checkbox' class='chk_agregar' title='agregar nuevos registros' id='optescribir_".$row['NAME_MODULO']."' name='access[".$row['COD_MODULO']."][2]' /> agregar";
                                    echo "<input type='checkbox' class='chk_editar' title='editar registros desde tablas' id='opteditar_".$row['NAME_MODULO']."' name='access[".$row['COD_MODULO']."][3]' /> editar";
                                echo "</td>";
                            }
