        ...
        <tbody>
            ...
            while($item_permiso = $lista_permisos->fetch(PDO::FETCH_ASSOC)){
                ...
                else
                {
                    ...
                    if($item_permiso['PARENT_MODULO'] != null && $item_permiso['PARENT_MODULO'] > 0){
                        echo "<td><span class='ui-icon ui-icon-carat-1-sw' style='float:left;'></span>".$item_permiso['LABEL_MODULO']."</td>";
                        
                        echo "<td class='options'> \n";
                            echo "<input type='checkbox' class='chk_ver' title='ver registros' id='optver_".$item_permiso['NAME_MODULO']."' ".$checked." ".$disabled." name='access[".$item_permiso['COD_MODULO']."][1]' />"; 
                        echo "</td>";
                    }
                    else{
                        echo "<td>".$item_permiso['LABEL_MODULO']."</td>";
                        ...
                        echo "<td class='options'> \n";
                            echo "<input type='checkbox' class='chk_ver' title='ver registros' id='optver_".$item_permiso['NAME_MODULO']."' ".$checked." ".$disabled." name='access[".$item_permiso['COD_MODULO']."][1]' /> ver"; 
                            echo "<input type='checkbox' class='chk_agregar' title='agregar nuevos registros' id=''optescribir_".$item_permiso['NAME_MODULO']."' ".$checked_write." ".$disabled." name='access[".$item_permiso['COD_MODULO']."][2]' /> agregar";
                            echo "<input type='checkbox' class='chk_editar' title='editar registros desde tablas' id='opteditar_".$item_permiso['NAME_MODULO']."' ".$checked_edit." ".$disabled." name='access[".$item_permiso['COD_MODULO']."][3]' /> editar";
                        echo "</td>";
                    }
                    echo "</tr>";
                }
